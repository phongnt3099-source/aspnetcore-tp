...
using ThienPhucDental.Test.Base.TestData;
...
namespace ThienPhucDental.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="ThienPhucDentalDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(ThienPhucDentalDbContext context)
            {
        ...
        protected void UsingDbContext(Action<ThienPhucDentalDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ThienPhucDentalDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<ThienPhucDentalDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<ThienPhucDentalDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ThienPhucDentalDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ThienPhucDentalDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ThienPhucDentalDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ThienPhucDentalDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<ThienPhucDentalDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<ThienPhucDentalDbContext, Task<TResult>> func)
        {
            TResult result;
