...
using ThienPhucDental.Web.Chat.SignalR;
using ThienPhucDental.Web.Common;
using ThienPhucDental.Web.Swagger;
using Stripe;
...
using ThienPhucDental.Schemas;
using ThienPhucDental.Web.HealthCheck;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using ThienPhucDental.Web.MultiTenancy;
using Abp.HtmlSanitizer;
using Microsoft.AspNetCore.Authentication.Cookies;
using ThienPhucDental.Web.OpenIddict;
...
namespace ThienPhucDental.Web.Startup
{
    public class Startup
    {
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            if (ThienPhucDentalConsts.PreventNotExistingTenantSubdomains)
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "ThienPhucDental API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("ThienPhucDental.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() {Title = "ThienPhucDental API", Version = "v1"});
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"ThienPhucDental.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"ThienPhucDental.Web.Core.xml";
